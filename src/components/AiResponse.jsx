...
const AiResponse = ({ aiResponse, children }) => {
  ...
  // Initialize codeTheme state to empty string, this will be used to store selected code theme.
  const [codeTheme, setCodeTheme] = useState('');

  const { showSnackbar, hideSnackbar } = useSnackbar();

  // Use useEffect to detect changes in user's preferred color scheme
  useEffect(() => {
    // Create media query to detect users preferred color scheme
    const mediaQuery = window.matchMedia('(prefers-color-scheme: dark)');

    // Initially set codeTheme based on current media query result
    setCodeTheme(mediaQuery.matches ? hopscotch : coy);

    // Create an event listener to handle changes in preferred color scheme.
  ...
  const code = ({ children, className, ...rest }) => {
    ...
    return match ? (
      <>
        ...
        <div
          className="bg-light-surfaceContainer dark:bg-dark-surfaceContainer rounded-t-extraSmall rounded-b-medium 
        flex justify-between items-center h-11 font-sans text-bodyMedium ps-4 pe-2"
        >
          <p>
            Use code
            <a
